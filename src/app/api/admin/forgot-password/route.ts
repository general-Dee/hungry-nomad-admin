...
// Every response below is padded to this minimum duration so that response
// timing can't reveal whether the email had an admin account -- otherwise a
// known email takes measurably longer (it makes an extra Resend API call)
// than an unknown one (which short-circuits after generateLink).
...
export async function POST(request: Request) {
  ...
  if (!process.env.RESEND_API_KEY || !fromAddress) {
    ...
    response = NextResponse.json({ error: GENERIC_ERROR }, { status: 500 });
  } else {
    ...
    if (linkError || !linkData?.properties?.hashed_token) {
      ...
      if (!resendRes.ok) {
        ...
        response = NextResponse.json({ error: GENERIC_ERROR }, { status: 502 });
      } else {
        response = NextResponse.json({ success: true });
      }
    }
